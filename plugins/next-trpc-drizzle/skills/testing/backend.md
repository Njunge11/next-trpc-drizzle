...
Stack: **Vitest** (`vitest.backend.config.mts`), **tRPC `createCaller`**, **fake repos** for services, **PGlite** (real schema) for repos. Tests live in each feature's `api/__tests__/` and `db/__tests__/`.
...
**Repo (`db/__tests__/`) — PGlite, real schema.** Reused dashboard tables need their FKs, so run against the dashboard migration set. Assert rows in/out of the DB, not which Drizzle calls ran.
```ts
const partner = await repo.createPartner({ name: "Acme", mpesaNumber: "…" })
expect(await repo.getPartnerByCode(partner.code)).toMatchObject({ name: "Acme" })
```
...
```ts
const svc = makeReferralsService({ repo: fakeRepo, now: () => FIXED, uuid: () => "id-1" })
await svc.bindReferral({ companyId: "c1", code: "ABC" })
expect(fakeRepo.commissions).toContainEqual(expect.objectContaining({ amountKes: 150 }))
```
...
await expect(caller.promo.markPaid({ partnerId: "x" })).rejects.toThrow(/below threshold/)
```
